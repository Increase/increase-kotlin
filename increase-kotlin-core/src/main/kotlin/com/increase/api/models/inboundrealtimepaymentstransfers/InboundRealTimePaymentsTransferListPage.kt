...
class InboundRealTimePaymentsTransferListPage private constructor(
    ...
    fun getNextPageParams(): InboundRealTimePaymentsTransferListParams? {
      ...
      return InboundRealTimePaymentsTransferListParams.builder().from(params).apply {nextCursor()?.let{ this.cursor(it) } }.build()
    }

    fun getNextPage(): InboundRealTimePaymentsTransferListPage? {
      return getNextPageParams()?.let {
          inboundRealTimePaymentsTransfersService.list(it)
      }
    }
    ...
    companion object {
        ...
        fun of(inboundRealTimePaymentsTransfersService: InboundRealTimePaymentsTransferService, params: InboundRealTimePaymentsTransferListParams, response: Response) =
    ...
    class AutoPager(
        private val firstPage: InboundRealTimePaymentsTransferListPage,

    ) : Sequence<InboundRealTimePaymentsTransfer> {

        override fun iterator(): Iterator<InboundRealTimePaymentsTransfer> =
            iterator {
                var page = firstPage
                var index = 0
                while (true) {
