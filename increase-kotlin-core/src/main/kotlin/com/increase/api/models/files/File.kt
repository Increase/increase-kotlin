...
/**
 * Files are objects that represent a file hosted on Increase's servers. Each File has a `purpose`
 * that determines how Increase will use it. You upload a File so it can be attached to another
 * object in the API (for example, when uploading a check image), or Increase creates one for you to
 * download (for example, an autogenerated statement PDF). If you need to download a File, use the
 * `/files/{file_id}/contents` endpoint. If you need to share a File with someone who doesn't have
 * access to your API key, create a File Link.
 */
class File
...
private constructor(
    ...
    override fun toString() =
        "File{id=$id, createdAt=$createdAt, description=$description, direction=$direction, filename=$filename, idempotencyKey=$idempotencyKey, mimeType=$mimeType, purpose=$purpose, type=$type, additionalProperties=$additionalProperties}"
}
